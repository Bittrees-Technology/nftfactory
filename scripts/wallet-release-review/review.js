...
async function checkWallet(){
 if(await request('eth_chainId')!=='0xaa36a7')throw Error('Select Sepolia in your wallet before continuing.');
...
function render(){
 $('transactions').replaceChildren(...manifest.transactions.map((t,i)=>{
  ...
  const small=document.createElement('small');small.textContent=`${t.contractAddress} · value: 0 Sepolia ETH (network fee applies)`;li.append(small);
...
$('connect').onclick=async()=>{try{
 ...
 if(await request('eth_chainId')!=='0xaa36a7')await request('wallet_switchEthereumChain',[{chainId:'0xaa36a7'}]);
...
$('next').onclick=async()=>{if(busy)return;busy=true;$('next').disabled=true;try{
 ...
 const hash=await request('eth_sendTransaction',[{from:manifest.signer,...(t.to?{to:t.to}:{}),data:t.input,value:'0x0',nonce:t.nonce,chainId:'0xaa36a7'}]);
 hashes.push(hash);
...
Promise.all([fetch('/manifest').then(r=>r.json()),fetch('/receipts').then(r=>r.json())]).then(([m,h])=>{manifest=m;hashes=h;$('signer').textContent=m.signer;$('safe').textContent=m.adminSafe;render();note(`${hashes.length} of ${m.transactions.length} requests recorded. Connect your wallet to continue.`);}).catch(()=>note('Could not load the reviewed deployment. Return to Codex.'));
