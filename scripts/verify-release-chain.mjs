...
const manifest=JSON.parse(await readFile('docs/reviews/product-expansion/sepolia-replacement/deployed-contracts.json','utf8'));
...
function clientFor(network){return createPublicClient({transport:custom({async request({method,params=[]}){
 if(!['eth_chainId','eth_blockNumber','eth_getBlockByNumber','eth_getCode','eth_call','eth_getBalance','eth_getTransactionCount','eth_gasPrice'].includes(method))throw Error('Read-only method required');
...
for(const [network,chainId] of [['eth-sepolia',11155111],['base-mainnet',8453],['robinhood-mainnet',4663]]){
 ...
 report.passed=report.custody.passed&&(report.runtime||[]).every(r=>r.exact)&&(report.configuration||[]).every(r=>r.passed)&&(report.simulations||[]).every(r=>r.passed);
