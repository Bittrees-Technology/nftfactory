...
contract Marketplace is Owned {
    ...
    function buy(uint256 listingId) external payable nonReentrant {
        ...
        if (registry.blocked(msg.sender) || registry.blocked(seller) || registry.blocked(nft) || blockedCollection[nft])
        {
            revert Sanctioned();
        }
        _assertPaymentTokenAllowed(paymentToken);
    ...
    function _settleEscrowedOffer(address paymentToken, address seller, uint256 price) internal {
        ...
        if (paymentToken == address(0)) {
            ...
            if (!sellerOk) revert EthTransferFailed();
            return;
        }
    ...
    function _standardKey(string memory standard) internal pure returns (bytes32 key) {
        ...
        if (key != _ERC721_KEY && key != _ERC1155_KEY) revert UnsupportedStandard();
    }

    function _normalizedStandard(bytes32 standardKey) internal pure returns (string memory) {
        return standardKey == _ERC721_KEY ? "ERC721" : "ERC1155";
    }
