...
export default function CreatorPageClient({address}:{address:string}) {
 ...
 {module==='featured'&&featured.length>0&&<><h2>Featured artwork</h2><div className="creatorArtworkGrid">{featured.map(item=><ArtworkCard key={item.id} item={item} readOnly={data.readOnly}/>)}</div></>}
 {module==='artwork'&&<><h2>Collected artwork</h2>{data.holdingsError ? <p role="status">Artwork is temporarily unavailable. Your creator page is still visible.</p> : items.length ? <div className="creatorArtworkGrid">{items.map(item=><ArtworkCard key={item.id} item={item} readOnly={data.readOnly}/>)}</div> : <p>No artwork is available yet. New mints appear after indexing completes.</p>}</>}
