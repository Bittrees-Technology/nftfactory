...
export default function ListingManagementCard({
  item,
  ipfsGateway,
  chainId,
  currentAddress,
  ...
  const appChain = getAppChain(effectiveChainId);
  const explorerBaseUrl = getExplorerBaseUrl(effectiveChainId);
  const wrongNetwork = isConnected && currentChainId !== effectiveChainId;
  const canBuy = !isMine && isConnected;
        ...
        <div className="feedCardContent">
          ...
          <div className="feedCardBody">
            <div className="feedCardMain">
              ...
              <p className="feedCardMetaLine">Created on {appChain.name} (chain {effectiveChainId})</p>
            </div>

            <div className="feedCardFacts">
              <DetailGridItem className="feedFact" labelClassName="feedFactLabel" label="Token" value={`#${item.tokenId.toString()}`} />
              <DetailGridItem className="feedFact" labelClassName="feedFactLabel" label="Amount" value={presentation.amountLabel} />
              <DetailGridItem className="feedFact" labelClassName="feedFactLabel" label="Price" value={presentation.priceLabel} />
              <DetailGridItem className="feedFact" labelClassName="feedFactLabel" label="Market" value={presentation.marketLabel} />
              <DetailGridItem className="feedFact" labelClassName="feedFactLabel" label="Ends" value={presentation.expiresAtLabel} />
            </div>
          </div>
        </div>
      </div>
          ...
          {variant === "mine" ? (
            <>
              ...
              <button
                ...
                disabled={isCanceling || !isConnected}
              >
                {isCanceling ? "Canceling..." : wrongNetwork ? `Switch to ${appChain.name}` : "Cancel"}
              </button>
            </>
          ) : (
            <button
              ...
              {isBuying
                ...
                : isMine
                  ...
                  : wrongNetwork
                    ...
                    : item.paymentToken === ZERO_ADDRESS
                      ? "Buy"
                      : "Buy (ERC20)"}
            </button>
          )}
