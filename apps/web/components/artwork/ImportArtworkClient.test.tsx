...
it('restores partially entered values after remount and clears only on request',()=>{
 ...
 fireEvent.change(screen.getByLabelText('Token IDs'),{target:{value:'1, 2,'}});
 ...
 expect((screen.getByLabelText('Token IDs') as HTMLTextAreaElement).value).toBe('1, 2,');
...
it('keeps wallet drafts isolated and preserves guest work on first connection',()=>{
 ...
 fireEvent.change(screen.getByLabelText('Token IDs'),{target:{value:'7'}});
 account.address='0x'+'11'.repeat(20);account.status='connected';view.rerender(<ImportArtworkClient/>);
 expect((screen.getByLabelText('Token IDs') as HTMLTextAreaElement).value).toBe('7');
 account.address='0x'+'22'.repeat(20);view.rerender(<ImportArtworkClient/>);
 expect((screen.getByLabelText('Token IDs') as HTMLTextAreaElement).value).toBe('');
 fireEvent.change(screen.getByLabelText('Token IDs'),{target:{value:'9'}});
