...
function CreateWorkspace({initialChainId}:{initialChainId:number}) {
  ...
  useEffect(() => {
    ...
    void loadArtworkDraft(scope).then(value => {
      if (active && value && value.status !== 'published') { setDraft(value); setStage(value.txHash ? 2 : 0); setDraftNotice(value.txHash ? 'A submitted mint was restored. Check its confirmation or start a new NFT; its receipt remains in your mint history.' : 'Your saved artwork draft was restored. Start a new NFT to clear it.'); }
  ...
  useEffect(() => { if (loaded) void saveArtworkDraft(draft, { chainId: targetChainId, wallet: address }).catch(() => setDraftNotice('Local saving is unavailable. Keep this page open to retain your draft.')); }, [draft, loaded, targetChainId, address]);
  async function startNew() {
    if (busyRef.current) return;
    try { await archiveArtworkDraft(draft, scope); }
    catch { setMessage('Could not save your receipt history. Keep your transaction hash before clearing this draft.'); return; }
    if (draft.txHash) setReceipts(current => [...current.filter(item => item.txHash !== draft.txHash), { ...draft, file: undefined }]);
    setDraft({ name: '', description: '' }); setComplete(false); setStage(0); setMessage('');
    setDraftNotice('New artwork. Your draft stays private on this device until you publish.');
  }
