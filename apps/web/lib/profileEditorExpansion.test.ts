...
it('renders whole-page slots while keeping scripts, forms, event handlers and frame injection disabled',()=>{
 const doc=profilePageDocument('0x123',{displayName:'<script>bad()</script>',avatarUrl:'https://images.example/avatar.png',design:{customScope:'page',customHtml:'{{intro}}{{top8}}<script>attack()</script><form><input></form>',customCss:'</style><script>attack()</script>',top8Targets:[{label:'Friend',url:'/profile/eth.friend'}]}},[]);
