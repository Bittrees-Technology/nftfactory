...
export function customProfileDocument(html:string,css:string,wholePage=false){
 const clean=sanitizeHtml(html.slice(0,wholePage?120000:20000),{
  allowedTags:[...(wholePage?['a','img']:[]),'section','article','header','footer','main','aside','div','span','p','h1','h2','h3','h4','h5','h6','strong','em','b','i','u','s','small','br','hr','ul','ol','li','blockquote','pre','code','figure','figcaption','details','summary','table','thead','tbody','tr','th','td'],
  allowedSchemes:['https','http'],
  transformTags:wholePage?{a:sanitizeHtml.simpleTransform('a',{target:'_blank',rel:'noopener noreferrer'})}:{},
  allowedAttributes:{a:wholePage?['href','target','rel']:[],img:wholePage?['src','alt','width','height','loading']:[],'*':['class','id','title','lang','dir'],'th':['colspan','rowspan'],'td':['colspan','rowspan']},
 ...
 return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'none'; style-src 'unsafe-inline'; img-src ${wholePage?'https:':"'none'"}; font-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'"><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{margin:0;padding:24px;font:16px/1.6 system-ui;color:#eee;background:#15182c;overflow-wrap:anywhere}*{box-sizing:border-box}pre{white-space:pre-wrap}@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation:none!important;transition:none!important}}${safeCss}</style></head><body>${clean}</body></html>`;
}
