...
it('retains an explicit collection association only in the collection action',async()=>{vi.stubGlobal('fetch',vi.fn().mockResolvedValue({ok:true,json:async()=>({address:owner})}));await linkCreatorIdentity({name:'work.artist.eth',source:'external-subname',ownerAddress:owner,collectionAddress:owner},vi.fn());expect(linkProfileIdentity).toHaveBeenCalledWith(expect.objectContaining({collectionAddress:owner}));});
