...
import { probeCollectionVerification, verifyCollectionContract } from "./collectionVerificationApi";
...
describe("collectionVerificationApi", () => {
  ...
  it("parses a successful verification status probe response", async () => {
    global.fetch = vi.fn(async () =>
      new Response(
        JSON.stringify({
          state: "verified",
    ...
    await expect(
      probeCollectionVerification(11155111, "0xeD8F6de4Cc63D4349042D1F9051a7E0882E9eB10")
    ).resolves.toMatchObject({
      state: "verified"
    });
  });
