...
export type ImportDraft={chainId:number;contract:string;ids:string};
...
export function readImportDraft(key:string):ImportDraft|null {
 ...
 try{const d=JSON.parse(raw);return d.version===1&&IMPORT_NETWORKS.some(n=>n.id===d.chainId)&&typeof d.contract==='string'&&d.contract.length<=42&&typeof d.ids==='string'&&d.ids.length<=800?{chainId:d.chainId,contract:d.contract,ids:d.ids}:null;}catch{return null;}
}
