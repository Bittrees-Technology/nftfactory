...
export async function ensureWalletSession(address: Address, signMessage: (args: { message: string }) => Promise<Hex>) {
  const current = await fetch('/api/auth').then(r => r.json());
  if (current.address?.toLowerCase() === address.toLowerCase()) return;
  const challengeResponse = await fetch('/api/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ address }) });
