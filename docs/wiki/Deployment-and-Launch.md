...
- [ ] `npm run check:release` passes (aggregates typechecks, tests, web build, secret scan, env checks, IPFS backend reachability, runtime health, and public route smoke checks when configured)
