...
The updated clean backend package (`e546bff`) installed with lifecycle scripts disabled and generated its Prisma client, matching the prepared installer path. It started on the isolated local database and passed real signed-wallet profile persistence, replay rejection and unauthorized-owner rejection. Package dependency audit returned zero vulnerabilities. The local outage/retry browser check passed after restarting this package. Production remained untouched; a private local staging copy was prepared for the final administrator step.

Wallet browser review found the native chooser obscured the WalletConnect QR modal. The chooser now closes before QR handoff and cannot reopen while connection is pending. The unobstructed QR was verified visually; canceling it reopens the chooser with a cancellation message and enabled retry choices. Missing injected-wallet detection now directs users to a wallet-enabled browser or WalletConnect. Four regressions pass (238 web tests total). Real mobile pairing/return and transaction signatures remain pending.
