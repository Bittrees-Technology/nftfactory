...
| Wallet/SIWE/ENS | Shared single-use nonces, domain/chain/expiry binding, EOA and real local ERC-1271 verification; actual signed profile/replay flow passes against packaged backend; transferred-name regression | Deployed smart-account, ENS expiry and mobile WalletConnect/rejection acceptance |
...
| Networks | Explicit network definitions, deployment chain/treasury guards, local deployment rehearsal and per-network checklist | Funded/signed deployments and canaries for Sepolia replacements, Base and Robinhood testnets, then production networks; no paid broadcasts assumed |
...
Latest feature checks: 234 web tests, 90 backend tests, 47 script tests, contract suite including the fee regressions, and six real local-RPC SIWE checks. GitHub also built the web application successfully. These builds use test configuration and must not be uploaded as production artifacts.
