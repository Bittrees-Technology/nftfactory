...
## Release acceptance
PR #45 merged to main at 241c988. Vercel deployment dpl_8G2k4LbhVCx2mgnLYo4wuu5PWvJ1 promoted to nftfactory.org. Acer runtime package aa7d93a78d3572cdea04348d538015fe35c48f37 was installed through the administrator installer; a read-only runtime check confirmed customScope=page and persisted Top 8 target normalization, and indexer health returned ok=true.

All GitHub checks passed (run 34682598231). The production image endpoint rejects unsigned POST requests with HTTP 401. Live Brave loaded the existing B0B0 profile with its original retro settings; uploads, socials, featured picker, linked Top 8 controls and pop-out preview are present. Opened the separate preview window and confirmed it displayed the saved profile. No profile data was changed or published in live acceptance. Actual authenticated image upload and selecting real artwork remain user acceptance checks; this wallet currently returned no indexed artwork.
